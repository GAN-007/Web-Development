...
class Audio{
  ...
  _render(){
    ...
    this.webgl.mesh_2.scale.x = 1 + frequencyAvg / 310;
    this.webgl.mesh_2.scale.y = 1 + frequencyAvg / 350;
    this.webgl.mesh_2.scale.z = 1 + frequencyAvg / 333;
...
class Webgl{
  ...
  createShader(){
    ...
    this.fragment = [
      ...
      "void main(){",
      "float f = smoothstep(0.0, 0.00002, vFrequency * vFrequency) * vFrequency;",//yellow=red...blue=aqua...
      "float yellow = min(1.0, baseColor.y + f * 1.9);",
      "float green = min(1.0, baseColor.g + f * 3.6);",
      "float blue = min(1.0, baseColor.b + f* 0.01);",
      "float sum = yellow + blue + green;",

      "blue = min(1.0, blue + 0.3);",
      "green = max(0.0, green - 0.1);",
      
      "float offsetSum = (sum - (yellow + blue + green) / 3.0) / 3.0;",
      
      "blue += offsetSum + min(vPos * 2.0, -0.2);",
      "yellow += offsetSum + min(vPos * 0.5, 0.2);",
      "green += offsetSum - vPos * max(0.3, vFrequency * 2.0);",
      
      "vec3 color;",

      "color = vec3(yellow, green, blue);",
